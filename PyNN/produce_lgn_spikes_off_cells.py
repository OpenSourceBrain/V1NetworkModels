...
x_values = np.linspace(-lx_cells/2, lx_cells/2, Ncells, endpoint=True) + half_lattice_x
y_values = np.linspace(-ly_cells/2, ly_cells/2, Ncells, endpoint=True) + half_lattice_y
number_of_layers = 4
...
for x in x_values:
    for y in y_values:
        ...
        for layer in xrange(number_of_layers):
            ...
            spike_trains[layer].append(spike_times_thin)
...
for layer in xrange(number_of_layers):
    ...
    output_filename = folder + str(contrast) + '_spike_train_off_layer' + str(layer) + format

    # Save spike train
    f = open(output_filename, "wb")
    cPickle.dump(spike_trains[layer], f, protocol=2)
    f.close()

# Save positions
positions_filename = folder + 'positions_off' + format
f = open(positions_filename, "wb")
