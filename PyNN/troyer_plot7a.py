...
from connector_functions import create_lgn_to_cortical
...
neurons = np.where(orientations_exc == 0)[0][0]

neurons_null = (orientations_exc == 90)
neurons_null = np.where(orientations_exc == 90)[0][0]
...
plt.title('Preferred Stimulus')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(gexc.times, gexc[:, neurons_null], label='exc')
plt.plot(ginh.times, ginh[:, neurons_null], label='inh')
plt.xlabel('Time (ms)')
plt.ylabel('Conductance (uS)')
plt.title('Null Stimulus')
plt.ylim([0, 0.2])
plt.legend()
